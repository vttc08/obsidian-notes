...
## Experiment
The setup has an already working VPN/proxy node that can be used in T&T Supermarket. Any modification will be built on-top of the pre-existing [VLESS](https://xtls.github.io/en/config/outbounds/vless.html)+[WS](https://xtls.github.io/en/config/transports/websocket.html)+TLS on Telus PureFibre network masquerading behind a Amazon AWS [CloudFront CDN](https://aws.amazon.com/cloudfront/). All servers are within the Telus network with a dynamic public IP of `66.186.x.x`, and internal subnet of `10.10.120.0/24`. For the clients, the tests are done on an Samsung Android 15, using software [V2RayNG](https://github.com/2dust/v2rayNG) v1.10.24.
